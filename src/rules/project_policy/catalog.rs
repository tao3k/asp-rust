...
pub(super) fn rules_by_id() -> BTreeMap<&'static str, AspRustRule> {
    [
        ...
        AspRustRule::new(
            ...
            PACK_ID,
            RustDiagnosticSeverity::Warning,
            "ASP Rust must stay in the test dependency graph",
            "Policy providers belong under dev-dependencies so ordinary builds and downstream package consumers do not compile ASP Rust.",
            labels("project-policy"),
        ),
        AspRustRule::new(
            RUST_PROJ_R007,
        ...
        AspRustRule::new(
            ...
            PACK_ID,
            RustDiagnosticSeverity::Warning,
            "Build-script ASP Rust gate should migrate",
            "ASP Rust policy should execute from cargo test rather than build.rs.",
            labels("project-policy"),
        ),
        AspRustRule::new(
            RUST_PROJ_R010,
        ...
        AspRustRule::new(
            ...
            PACK_ID,
            RustDiagnosticSeverity::Warning,
            "Reserved legacy cargo-check gate rule",
            "Legacy cargo-check policy activation is superseded by the cargo-test gate.",
            labels("project-policy"),
        ),
        AspRustRule::new(
            RUST_PROJ_R012,
            PACK_ID,
            RustDiagnosticSeverity::Warning,
            "ASP Rust cargo-test policy gate is incomplete",
            "ASP Rust-enabled packages must mount a cargo-test policy gate backed by a direct provider or workspace Build Support dev-dependency.",
            labels("project-policy"),
        ),
        AspRustRule::new(
            RUST_PROJ_R013,
        ...
        AspRustRule::new(
            ...
            PACK_ID,
            RustDiagnosticSeverity::Warning,
            "Workspace member dev-gate wrapper alias is redundant",
            "Expose one canonical workspace member dev-gate entrypoint; duplicate public aliases need deprecation metadata and a bounded migration plan.",
            labels("project-policy"),
        ),
        AspRustRule::new(
            RUST_PROJ_R020,
